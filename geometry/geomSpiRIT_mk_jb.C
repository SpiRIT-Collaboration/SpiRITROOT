...
void geomSpiRIT_mk_jb()
{
 ...
 // geoIF -> setMediaFile(dirGeom + "media.geo");
  ...
  // ----------------------------------------------------
  // defintion of active veto scintillators
  // dimensions in cm
  // ----------------------------------------------------

  Double_t dlActiveVeto = 9.;
  Double_t dwActiveVeto = 5;
  Double_t dtActiveVeto = 0.6;

  TGeoVolume * ActiveVetoL = gGeoManager -> MakeBox("ActiveVetoL",polyvinyltoluene,dwActiveVeto/2.,dlActiveVeto/2.,dtActiveVeto/2.);
  TGeoVolume * ActiveVetoR = gGeoManager -> MakeBox("ActiveVetoR",polyvinyltoluene,dwActiveVeto/2.,dlActiveVeto/2.,dtActiveVeto/2.);
  // swap width and length for up and down
  TGeoVolume * ActiveVetoU = gGeoManager -> MakeBox("ActiveVetoU",polyvinyltoluene,dlActiveVeto/2.,dwActiveVeto/2.,dtActiveVeto/2.);
  TGeoVolume * ActiveVetoD = gGeoManager -> MakeBox("ActiveVetoD",polyvinyltoluene,dlActiveVeto/2.,dwActiveVeto/2.,dtActiveVeto/2.);


  Double_t offxActiveVetoL = -2.1 - dwActiveVeto/2.;
  Double_t offxActiveVetoR = +0.5 + dwActiveVeto/2.;


  Double_t offzActiveVetoLR =  offzPadPlane + dzPadPlane/2. + 24.19 - dtActiveVeto/2.;
  Double_t offzActiveVetoUD =  offzPadPlane + dzPadPlane/2. + 22.59 - dtActiveVeto/2.;


  ///not 100% sure of these, I want offyPadPlane to put us a the center of the magnet in y
  Double_t offyActiveVetoU = offyPadPlane-20.5 + 3.9/2. + dwActiveVeto/2.;
  Double_t offyActiveVetoD = offyPadPlane-20.5 - 3.9/2. - dwActiveVeto/2.;
  Double_t offyActiveVetoLR = offyPadPlane-20.5;

  TGeoTranslation * transActiveVetoL = new TGeoTranslation("transActiveVetoL",offxActiveVetoL,offyActiveVetoLR,offzActiveVetoLR);
  TGeoTranslation * transActiveVetoR = new TGeoTranslation("transActiveVetoR",offxActiveVetoR,offyActiveVetoLR,offzActiveVetoLR);
  TGeoTranslation * transActiveVetoU = new TGeoTranslation("transActiveVetoU",0.,offyActiveVetoU,offzActiveVetoUD);
  TGeoTranslation * transActiveVetoD = new TGeoTranslation("transActiveVetoD",0.,offyActiveVetoD,offzActiveVetoUD);

  transActiveVetoL -> RegisterYourself();
  transActiveVetoR -> RegisterYourself();
  transActiveVetoU -> RegisterYourself();
  transActiveVetoD -> RegisterYourself();


  ActiveVetoL -> SetMedium(polyvinyltoluene);
  ActiveVetoR -> SetMedium(polyvinyltoluene);
  ActiveVetoU -> SetMedium(polyvinyltoluene);
  ActiveVetoD -> SetMedium(polyvinyltoluene);
 ...
 TGeoCompositeShape * p10gasComposite = new TGeoCompositeShape("p10gasComposite","p10BoxVol-activeComposite-encFrameComposite-cageFrontComposite-cageSideComposite-frontWindow:transFrontWindow-frontWindowCradleComposite-backWindowFrameComposite-topFrameComposite-bottomPlateComposite-FWFR:transFWFR-FWFL:transFWFL-FWFT:transFWFT-FWFB:transFWFB-FWF2R:transFWF2R-FWF2L:transFWF2L-ActiveVetoL:transActiveVetoL-ActiveVetoR:transActiveVetoR-ActiveVetoU:transActiveVetoU-ActiveVetoD:transActiveVetoD");
  ...
  TGeoVolume * katanaBox = gGeoManager -> MakeBox("katanaBox",air,(offxKatanaMPlaToPla*12.+12.)/2.,dlKatanaM/2.,dtKatanaM /2.);
  TGeoTranslation * transKatanaBox = new TGeoTranslation("transKatanaBox",offxKatanaM+offxKatanaMPlaToPla,offyKatanaM,offzKatanaM);
  transKatanaBox -> RegisterYourself();
  
  TGeoCompositeShape * outerAirComposite = new TGeoCompositeShape("outerAirComposite","outerAirBox-tpcEncBox-kyotoBox:transKyotoBoxL-kyotoBox:transKyotoBoxR-katanaVPlaL:transKatanaVPlaL-katanaVPlaM:transKatanaVPlaM-katanaVPlaR:transKatanaVPlaR-katanaBox:transKatanaBox");
  ...
  tpc -> AddNode(ActiveVetoL,1,transActiveVetoL);
  tpc -> AddNode(ActiveVetoR,1,transActiveVetoR);
  tpc -> AddNode(ActiveVetoU,1,transActiveVetoU);
  tpc -> AddNode(ActiveVetoD,1,transActiveVetoD);
  ...
  for(Int_t ikatanaL=0; ikatanaL<5; ikatanaL++)
    tpc -> AddNode(katanaMPlaL[ikatanaL],1,transKatanaML[ikatanaL]);
  for(Int_t ikatanaR=0; ikatanaR<7; ikatanaR++)
    tpc -> AddNode(katanaMPlaR[ikatanaR],1,transKatanaMR[ikatanaR]);
  ...
  ActiveVetoL -> SetTransparency(transparency);
  ActiveVetoR -> SetTransparency(transparency);
  ActiveVetoU -> SetTransparency(transparency);
  ActiveVetoD -> SetTransparency(transparency);
  
  p10gas            -> SetTransparency(95);
  outerAir          -> SetTransparency(95);


  ActiveVetoL -> SetLineColor(30);
  ActiveVetoR -> SetLineColor(30);
  ActiveVetoU -> SetLineColor(30);
  ActiveVetoD -> SetLineColor(30);

  for(Int_t ikatanaL=0; ikatanaL<5; ikatanaL++)
    katanaMPlaL[ikatanaL] -> SetLineColor(7);
  for(Int_t ikatanaR=0; ikatanaR<7; ikatanaR++)
    katanaMPlaR[ikatanaR] -> SetLineColor(7);
