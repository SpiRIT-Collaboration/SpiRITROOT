...
public: 
	...
	virtual void SetReader(TTreeReader& t_reader) override
	{
		myTrackArray_ = std::unique_ptr<ReaderValue>(new ReaderValue(t_reader, "STRecoTrack"));
		this->SetMyReader(t_reader);
		if(NextRule_) NextRule_->SetReader(t_reader);
	};
...
protected:
	std::unique_ptr<ReaderValue> myTrackArray_;
	STRecoTrack* track_; 
	unsigned track_id_;
};
...
protected:
	const int x_, y_;
	std::unique_ptr<ReaderValue> myTrackArray_;
	std::unique_ptr<ReaderValue> myHitArray_;
};
...
public: 
	...
	{myEmbedArray_ = std::unique_ptr<ReaderValue>(new ReaderValue(t_reader, "STEmbedTrack"));}
...
protected:
	const int x_, y_;
	std::unique_ptr<ReaderValue> myEmbedArray_;
};
...
public:
	...
	MomentumTracks(int t_axis=0) : axis_(t_axis) { fill_[1] = 0; };
...
public:
	// which axis x y z or magnitude of the momentum do you want to plot
	MomentumEmbedTracks(int t_axis=0) : axis_(t_axis) { fill_[1] = 0; };
...
protected:
	int axis_;
	std::unique_ptr<ReaderValue> myTrackArray_, myEmbedArray_;
};
...
public: 
	...
	virtual void SetMyReader(TTreeReader& t_reader) override
	{myHitArray_ = std::unique_ptr<ReaderValue>(new ReaderValue(t_reader, "STHit"));}
...
public: 
	...
	virtual void SetMyReader(TTreeReader& t_reader) override
	{myHitArray_ = std::unique_ptr<ReaderValue>(new ReaderValue(t_reader, "STEmbedHit"));}
...
{
public:
	ValueCut(double t_lower, double t_upper) : lower_(t_lower), upper_(t_upper)
	{}
	...
	virtual void Selection(DataSink& t_hist, unsigned t_entry) override
	{
		if( upper_ >= fill_[1] && fill_[1] >= lower_) 
			this->FillData(t_hist, t_entry);
	}
protected:
