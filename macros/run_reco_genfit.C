...
void run_reco_genfit
(
  TString          name = "riemann_cocktail",
  TString      dataFile = "",
...
)
{
  ...
  */
  ...
  STGenfitTask *fGenfitTask = new STGenfitTask();
  fGenfitTask -> SetPersistence(kTRUE);
  fGenfitTask -> SetDisplay(); // <-- Comment out this for disable display
  fGenfitTask -> SetFindVertex();
  fRun -> AddTask(fGenfitTask);
