...
const int num_jobs_in_queue = 30; // only allow this amount of jobs on ember for other users

void run_general_mc(const std::string& t_config_list, int t_start_from = 0, int t_num_jobs_to_be_submitted = num_jobs_in_queue)
{
	...
	for(unsigned i = t_start_from; i < config_list.Size(); ++i)
	{
		...
		TString command = TString::Format("sbatch ./embedMacros/submit_general_mc.sh \"%s\" %d \"%s\" \"%s\"", output_name.Data(), i + num_jobs_in_queue, t_config_list.c_str(), input_name.Data());
