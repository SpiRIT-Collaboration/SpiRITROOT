...
void run_general_all(const std::string& t_config_list, int t_start_from = 0, int t_num_jobs_to_be_submitted = num_jobs_in_queue)
{
	...
	for(unsigned i = t_start_from; i < config_list.Size(); ++i)
	{
		...
		int fd = mkstemp(tempname4reco);
		logdir.ReplaceAll("/", "\\/");
		TString command = TString::Format("sed \'s/LOGDIRTOBESUB/%s/g\' %s/macros/embedMacros/submit_general_allreco.sh > %s", logdir.Data(), workDir.Data(), tempname4reco);
		system(command.Data());
		command = TString::Format("chmod 755 %s", tempname4reco);
		system(command.Data());
		close(fd);

                fd = mkstemp(tempname4mc);
		command = TString::Format("sed \'s/LOGDIRTOBESUB/%s/g\' %s/macros/embedMacros/submit_general_allmc.sh > %s", logdir.Data(), workDir.Data(), tempname4mc);
		system(command.Data());
		command = TString::Format("chmod 755 %s", tempname4mc);
		system(command.Data());
		close(fd);

		command = TString::Format("%s/macros/embedMacros/submit_general_all.sh \"%s\" \"%s\" %d \"%s\" \"%s\" \"%s\" \"%s\" \"%s\" \"%s\" --export=VMCWORKDIR=$VMCWORKDIR", workDir.Data(), tempname4reco, output_name.Data(), i + num_jobs_in_queue, t_config_list.c_str(), input_name.Data(), DESTINATION.c_str(), FISHTANK_USER.c_str(), par_name.c_str(), tempname4mc);
