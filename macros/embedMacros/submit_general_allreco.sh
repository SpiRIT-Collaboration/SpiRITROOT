...
SPLIT=$((3*SLURM_ARRAY_TASK_ID+0)); scp -o ProxyCommand="ssh -W %h:%p $FISHTANKUSER@nsclgw1.nscl.msu.edu" $VMCWORKDIR/macros/data/${OUTPUT}/*s$(printf "%02d" $SPLIT)* $FISHTANKUSER@fishtank:$DESTINATION/$OUTPUT; rm $VMCWORKDIR/macros/data/${OUTPUT}/*s$(printf "%02d" $SPLIT)*
SPLIT=$((3*SLURM_ARRAY_TASK_ID+1)); scp -o ProxyCommand="ssh -W %h:%p $FISHTANKUSER@nsclgw1.nscl.msu.edu" $VMCWORKDIR/macros/data/${OUTPUT}/*s$(printf "%02d" $SPLIT)* $FISHTANKUSER@fishtank:$DESTINATION/$OUTPUT; rm $VMCWORKDIR/macros/data/${OUTPUT}/*s$(printf "%02d" $SPLIT)*
SPLIT=$((3*SLURM_ARRAY_TASK_ID+2)); scp -o ProxyCommand="ssh -W %h:%p $FISHTANKUSER@nsclgw1.nscl.msu.edu" $VMCWORKDIR/macros/data/${OUTPUT}/*s$(printf "%02d" $SPLIT)* $FISHTANKUSER@fishtank:$DESTINATION/$OUTPUT; rm $VMCWORKDIR/macros/data/${OUTPUT}/*s$(printf "%02d" $SPLIT)*


cd ${VMCWORKDIR}/macros
if (( SLURM_ARRAY_TASK_ID == 0 )); then
    root ${VMCWORKDIR}\/macros\/embedMacros/run_general_all.C\(\"$CONFIGLIST\"\,$NEXTJOB,1\) -q -l > ${LOGDIR}_run.log
fi
