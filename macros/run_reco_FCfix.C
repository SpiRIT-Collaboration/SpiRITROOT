...
)
{
  ...
  auto hitMacro = new STMacroTask();
  hitMacro->AddInitFunction(hitSelect::Init);
  hitMacro->AddFunction(hitSelect::Exec);

  auto helix = new STHelixTrackingTask();
  helix -> SetInputBranch("STHit");
