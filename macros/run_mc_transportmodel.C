 ...
 * - How To Run
 *   In bash,
 *   > root 'run_mc.C("name", nEvents, kFALSE)'
 *   You do not need to open this file to change variables.
 *
 * - Varialbles
 *   @ name : Name of simulation. Just for indication.
     @ nEvents : number of loop for simulation. If not set, loop number will be loaded from event generator.
 *   @ useFieldMapFile : kFALSE to use constant field, with 0.5T. 
 ...
 )
{
  ...
  // -----------------------------------------------------------------
  // Run
  if(nEvents==-1) nEvents = fEvent->GetNEvents();
  fRun -> Run(nEvents);
