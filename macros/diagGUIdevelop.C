...
private:
    ...
    TString fCutFile = "IDcutFile.root";
    TCutG *fpimiCut = nullptr;

    Int_t fRunNum = 1045;
    Int_t fClustCut = 15;
    Int_t fCurrentTab = 0;
    Double_t fPOCACut = 20;
    Double_t fVertZPosCut = -21.0;
...
DiagnosticsGUI::DiagnosticsGUI(const TGWindow *p, UInt_t w, UInt_t h) {
   ...
   // Frame for using a vertex Z cut
    ...
    fRunEnt = new TGTextEntry(hframe);
...
void DiagnosticsGUI::ProcessRun() {
   ...
   std::string runsText = fRunEnt->GetText();

   std::istringstream iss(runsText);
   ...
   if(!listGood) {
      cout << "Invalid run list input!" << endl;
      cout << "Must be a space delimited list of integers less than 9999!" << endl;
      return;
   }
   ...
   for(auto run : runList) {
      ...
      TString testPath = TString::Format("data/run%04d_s00.%s.root", run, format.Data());
      TFile *file = TFile::Open(testPath);
      if(!file || file->IsZombie()) {
         auto message = TString::Format("Error: Run %04d has not been unpacked! Unpack before plotting.", run);
      ...
      TString filePath = TString::Format("data/run%04d_s*.%s.root", run, format.Data());
        ...
        for(int r = 0; r < trackCount; r++) {
            ...
            if(recoPtr != NULL) {
               if(recoPtr->GetNumLayerClusters() + recoPtr->GetNumRowClusters() > fClustCut && (recoPtr->GetPOCAVertex() - vertex).Mag() < fPOCACut) {
                  ...
                  if(fpimiCut->IsInside(mom * gfCharge, dedx)) {
                     pimi++;
                  }
                  if(fpimiCut->IsInside(mom * gfCharge * -1, dedx)) {
                     pipu++;
                  }
    ...
    cout << "Target Hits: " << targetHits << endl;
    cout << "pi-: " << pimi << endl;
    cout << "pi+: " << pipu << endl;
    DrawPID(0);
    DrawTP(1);
...
void DiagnosticsGUI::Init() {
    ...
    TFile *cutFile = new TFile(fCutFile);
    if(cutFile->IsOpen())
       cout << "cutFile is open" << endl;
    fpimiCut = (TCutG *)cutFile->Get("pimiCut");
    //cutFile->Close();

    if(fpimiCut == nullptr)
       cout << "Warning! pimi cut is nullptr!" << endl;
