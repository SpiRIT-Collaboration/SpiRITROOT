void run_mc_map(Int_t    nEvents  = 1,
                  Int_t    particle = 0,
                  Double_t momentum = 300.0, // [MeV/c]
                  TString  tag      = "map")
{
  momentum *= 0.001;
  Int_t particleID;
...
// Samurai Field Map implimentation, SamuraiMap_0.5T.dat file should be placed in the "input" directory
 STFieldMap *fMagField = new STFieldMap("SamuraiMap_0.5T","A");
 fMagField->SetPosition(-130.55/2,-51.10/2,-159.64/2);


  FairBoxGenerator* boxGen1 = new FairBoxGenerator(particleID, 2);           //Use Proton
