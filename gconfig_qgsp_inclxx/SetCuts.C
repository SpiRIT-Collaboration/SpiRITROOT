...
void SetCuts()
{
  ...
  gMC->SetProcess("CKOV",2); /// Cerenkov production
  gMC->SetProcess("RAYL",1); /// Rayleigh scattering

  Double_t cutC = 1.0E-4;
  Double_t cutB = 1.0E-3;
  Double_t cut1 = 1.0E-5;   // [GeV]
  Double_t cute = 1.0E-3;   // [GeV]
  Double_t tofmax = 1.E1;  // seconds

  cout << "SetCuts Macro: Setting cuts.." <<endl;
  
  gMC->SetCut("CUTGAM",cutC);   /// gammas (GeV)
  gMC->SetCut("CUTELE",cute);   /// electrons (GeV)
  ...
  gMC->SetCut("DCUTE",cute);    /// delta-rays by electrons (GeV)
  gMC->SetCut("DCUTM",cutB);    /// delta-rays by muons (GeV)
  gMC->SetCut("PPCUTM",cutB);   /// direct pair production by muons (GeV)
